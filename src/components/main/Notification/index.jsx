...
const Notification = ({ isAuth }) => {
    ...
    useEffect(() => {
        socket.on('newNotification', ({ notification, count }) => {
            console.log('STATE: ', notifications);
            setUnreadCount(unreadCount + 1);
            setNotifications((prev) => ({ ...prev, items: [notification, ...prev.items] }));
    ...
    const handleClickOutside = (e) => {
        const toggle = (e.target).closest('.notification-toggle');
    ...
    const handleReadNotification = async (id) => {
        try {
            ...
            const updatedNotifs = notifications.items.map((notif) => {
                if (notif.id === id) {
                    return {
                        ...notif,
                        unread: state
